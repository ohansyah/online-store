                ...
                <form wire:submit.prevent="save" enctype="multipart/form-data">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 p-6 lg:p-8">
                        <div>
                            ...
                            <input type="datetime-local" wire:model="started_at"
                                class="w-full p-2 border border-gray-300 rounded-lg mt-2" placeholder="Start At">
                            @error('started_at') <span class="text-red-500">{{ $message }}</span> @enderror

                            <input type="datetime-local" wire:model="ended_at"
                                class="w-full p-2 border border-gray-300 rounded-lg mt-2" placeholder="End At">
                            @error('ended_at') <span class="text-red-500">{{ $message }}</span> @enderror
